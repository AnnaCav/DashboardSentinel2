...
var callbackFunction = function getWeather(data) {
	...
	try {
		if (count == 0) throw "The GPS is not able to find yout location. Please try it again or type in the city name";
	}
	catch (err) {
		alert(err);
		window.location.href = "WetterV8.html";
	}
...
var callbackFunction = function getWeather(data) {
	...
	try {
		if (count == 0) throw "This city does not exist. Please try it again.";
	}
	catch (err) {
		alert(err);
		window.location.href = "WetterV8.html";
